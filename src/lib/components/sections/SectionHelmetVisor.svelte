<script>
  ...
  const ENTRY_END         = 0.045 // fraction of progress for the entry rise (smaller = helmet comes up sooner → less white gap before it). Kept low so the rise eats little scroll; ENTRY_START_Y must stay offscreen because p is clamped to 0 for the whole biography.
  const ENTRY_START_Y     = 100   // vh: helmet starts fully offscreen below = smooth glide (must match the p===0 offscreen value to avoid a jump AND to keep the helmet hidden behind the biography). Use ENTRY_END to tune how soon it settles.
  const CAM_FAR           = 8.5
  const CAM_CLOSE         = 1.8
